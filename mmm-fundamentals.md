...
A matching estimator can estimate the causal effect of a treatment (e.g., running an ad) by comparing treated and untreated units (e.g., time periods) that are similar in observed characteristics (e.g., other ad channels, seasonal effects, economic indicators).

> Side Note: Matching estimator — An intuitive explanation: Imagine you’re an analyst at a company that runs TV ads and you want to measure whether TV advertising increases sales. You have weekly sales data and know which weeks had TV ads (treated weeks) and which weeks didn’t (control weeks). Instead of simply comparing all weeks with TV ads vs. all weeks without TV ads (which could be biased), the matching estimator finds control weeks that closely resemble treated weeks in terms of other factors that influence sales, such as: Digital ad spend (e.g., Facebook, YouTube, Google Ads), Seasonality (e.g., Black Friday, holiday periods), Pricing & promotions, etc.
...
> Side Note: Matching estimator assumption & drawbacks — in simpler terms: Imagine you work for an e-commerce company and want to measure how YouTube ads impact sales. You plan to use a matching estimator, comparing weeks with YouTube ads against weeks without YouTube ads. For the matching estimator to work, it assumes that for every treated week (where YouTube ads ran), there is at least one control week (where YouTube ads didn’t run) that has the same values for other factors affecting sales. Each treated week must be matched to a control week with similar values for other factors (e.g., Facebook spend, seasonality, promotions). If no suitable match exists, that treated week might not have a valid comparison. note that if many treated weeks share similar characteristics, they might all match to the same control week. When media variables are correlated, its difficult to make clean comparisons, making the matching estimator unreliable for isolating the effect of a single 1channel. For example, if whenever YouTube ads are increased, Facebook and Google ads also increase (because marketing campaigns tend to scale across multiple platforms). Conversely, when YouTube ads are off, Facebook and Google ads are also low — thus making the channels correlated. If all weeks with YouTube ads also had high Facebook and Google ad spend, then there are no control weeks where YouTube ads were missing but Facebook/Google spend remained the same (high). This makes it impossible to find a clean comparison, meaning we can’t isolate the effect of YouTube ads because every time YouTube was on, so were other ad channels. When the number of control variables increases, the the number of unique control combinations explodes exponentially. If we add more control variables (like pricing levels, promotions, email marketing), the number of combinations to match rapidly becomes too large.
